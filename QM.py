...
def findminterms(a): #Function for finding out which minterms are merged. For example, 10-1 is obtained by merging 9(1001) and 11(1011)
    ...
    for i in range(pow(2,gaps)):
        ...
        for j in x[0]:
            if ind != -1:
                ind = ind+temp2[ind+1:].find('x')+1
            else:
                ind = temp2[ind+1:].find('x')
...
###INT MAIN OR WHATEVER
###what is python anyway

##input in form m(x,y,z)+d(a,b,c)
##can handle spaces as well

#take input
minterms = input("Enter the minterms + don't cares: ")

sep = '+'
#split minterms and don't cares (with the plus)
mt, sep, dc = minterms.partition('+')

#only minterms, not don't cares
mt = "".join(mt.split())
mt = "".join(c for c in mt if c not in 'md()')
mt = [int (i) for i in mt.split(',') if i.isdigit()]
mt.sort()

#dont cares
dc = "".join(dc.split())
dc = "".join(c for c in dc if c not in 'md()')
dc = [int (i) for i in dc.split(',') if i.isdigit()]
dc.sort()

minterms = mt + dc
minterms.sort()

#note the length of the longest binary string, we subtract two 
#because python automatically puts the prefix "0b" in front of the actual binary
size = len(bin(minterms[-1]))-2


##maxterms are just everything but minterms and don't cares
maxterms = []
...
#set groups to dictionary, sort by number of 1's, then we'll have minterms sorted by number of ones
groups = {} 

all_pi = set()

#Group minterms by number of ones. Cutoff the "0b" prefix, then fill with 0s if we need to. 
#Ex: we are sorting up to 4 bits, 1 (decimal) becomes "0b1", then we chop off the "0b", becoming "1"
#then, we need to fill with three zeroes up to "0001". 
...
#print sorted minterms
print("\n\nGroup (# of 1's)\tMinterms\t Minterms in Binary\n%s"%('='*59))
for i in sorted(groups.keys()):
    print("%5d:"%i) # Prints group number (aka number of 0s)
    for j in groups[i]:
        print("\t\t\t    %-20d%s"%(int(j,2),j)) # Prints minterm and its binary representation
    print('-'*59)

#start sorting groups into cubes
while True:
    tmp = groups.copy()
        ...
        for j in tmp[l[i]]: # Loop which iterates through current group elements
            for k in tmp[l[i+1]]: # Loop which iterates through next group elements
                ...
                if res[0]: # If the minterms differ by 1 bit only
                    try:
                        groups[m].append(j[:res[1]]+'x'+j[res[1]+1:]) if j[:res[1]]+'x'+j[res[1]+1:] not in groups[m] else None # Put a 'x' in the changing bit and add it to corresponding group
                    except KeyError:
                        groups[m] = [j[:res[1]]+'x'+j[res[1]+1:]] # If the group doesn't exist, create the group at first and then put a 'x' in the changing bit and add it to the newly created group
                    should_stop = False
...
#now start grouping maxterms
groups = {} 

all_pi = set()

size = len(bin(maxterms[-1]))-2
...
#print sorted minterms
print("\n\nGroup (# of 1's)\t Maxterms\t Maxterms in Binary\n%s"%('='*59))
for i in sorted(groups.keys()):
    print("%5d:"%i) # Prints group number (aka number of 0s)
    for j in groups[i]:
        print("\t\t\t    %-20d%s"%(int(j,2),j)) # Prints minterm and its binary representation
...
# Printing and processing of Prime Implicant chart starts
sz = len(str(maxterms[-1])) # The number of digits of the largest minterm
chart = {}
print('\n\n\nPrime Implicants chart:\n\n    Maxterms    |%s\n%s'%(' '.join((' '*(sz-len(str(i))))+str(i) for i in maxterms),'='*(len(maxterms)*(sz+1)+16)))
for i in all_pi:
    merged_minterms,y = findminterms(i),0
    print("%-16s|"%','.join(merged_minterms),end='')
    for j in refine(merged_minterms,dc):
        x = maxterms.index(int(j))*(sz+1) # The position where we should put 'X'
        print(' '*abs(x-y)+' '*(sz-1)+'X',end='')
        y = x+sz
        try:
            chart[j].append(i) if i not in chart[j] else None # Add minterm in chart
...
if(len(chart) == 0): # If no minterms remain after removing EPI related column
    EPI = [i.replace('0','t') for i in EPI] #maxterms, so have to switch bits from 0 to 1
    EPI = [i.replace('1','0') for i in EPI] #maxterms, so have to switch bits from 0 to 1
    EPI = [i.replace('t','1') for i in EPI] #maxterms, so have to switch bits from 0 to 1
    final_result = [findVariables(i) for i in EPI] # Final result with only EPIs
else: # Else follow Petrick's method for further simplification
    P = [[findVariables(j) for j in chart[i]] for i in chart]
    while len(P)>1: # Keep multiplying until we get the SOP form of P
    ...
    final_result = [min(P[0],key=len)] # Choosing the term with minimum variables from P
    EPI = [i.replace('0','t') for i in EPI] #maxterms, so have to switch bits from 0 to 1
    EPI = [i.replace('1','0') for i in EPI] #a little trickery here with a temp variable
    EPI = [i.replace('t','1') for i in EPI] #to flip all bits
    final_result.extend(findVariables(i) for i in EPI) # Adding the EPIs to final solution
